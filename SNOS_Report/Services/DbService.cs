...
namespace SNOS_Report.Services
{
    ...
    {
        private SND_SNOSEntities _context;

        public DbService()
        {
            _context = new SND_SNOSEntities();
        }
        ...
        public List<Error_Total> GetErrorByMonth(int line, string lang, int month, int year)
        {
            ...
            var result = getError
                .Join(
                    _context.Error_Mapping,                              // ตารางที่จะ Join
                    e => new { Error_No = e.Error_Num, Line_Type = line, Lang = lang }, // Key จาก getError
                    m => new { Error_No = m.Error_No, Line_Type = m.LINE_TYPE, Lang = m.Language }, // Key จาก Error_Mapping
                    (e, m) => new Error_Total                           // ผลลัพธ์ที่ต้องการ
                    {
                        month = e.month,
                        year = e.year,
                        Line = line,
